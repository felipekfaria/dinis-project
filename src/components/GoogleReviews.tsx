...
const ReviewCard = ({
  ...
  return <div className="bg-white p-4 rounded-lg h-full shadow-md flex flex-col">
      <div className="flex items-center gap-3 mb-3">
        <div className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center overflow-hidden flex-shrink-0">
          {review.avatar ? <img src={review.avatar} alt={review.name} className="w-full h-full object-cover" /> : <span className="text-white font-bold">{review.name.charAt(0)}</span>}
        </div>
        <div>
      ...
      <div className="flex-grow">
        <p className="text-sm text-gray-600 leading-relaxed">
          {isLongComment && !isExpanded ? `${review.comment.substring(0, characterLimit)}...` : review.comment}
        </p>
      </div>
      
      {isLongComment && <button onClick={() => setIsExpanded(!isExpanded)} className="text-xs text-blue-600 font-semibold mt-2 self-start hover:underline">
...
const DesktopLayout = ({
  ...
  scrollPrev,
  scrollNext
}: any) => <section className="py-24 bg-[#163357] overflow-hidden">
    <div className="container mx-auto px-4">
      <div className="relative max-w-4xl mx-auto">
        <div className="absolute top-1/2 -translate-y-1/2 right-0 translate-x-[80%] w-[750px] z-0 hidden lg:block">
          <img src={consultantImage2} alt="" aria-hidden="true" className="w-full h-auto" />
        </div>
        ...
        <div className="relative z-10">
          ...
          <div className="relative bg-[#cde4f0] p-6 sm:p-8 rounded-2xl">
            <div className="text-center mb-8 text-gray-800">
              ...
              <p className="text-gray-600 font-semibold">Com base em mais de 20 avaliações</p>
            ...
            <div className="overflow-hidden" ref={emblaRef}>
              <div className="flex -mx-3">
                {reviews.map((review: any) => <div key={review.id} className="flex-[0_0_100%] sm:flex-[0_0_50%] lg:flex-[0_0_33.333%] px-3">
...
const MobileLayout = ({
  ...
  scrollSnaps,
  selectedIndex
}: any) => <section className="py-16 bg-[#163357]">
    <div className="container mx-auto px-4">
      <div className="flex justify-center mb-8">
        <img src={consultantImage2} alt="Consultor Imobiliário" className="w-full max-w-xs h-auto" />
      </div>
      ...
      <div className="relative bg-[#cde4f0] p-6 sm:p-8 rounded-2xl max-w-4xl mx-auto">
        ...
        <div className="overflow-hidden mt-4" ref={emblaRef}>
          <div className="flex -mx-3">
            {reviews.map((review: any) => <div key={review.id} className="flex-[0_0_100%] sm:flex-[0_0_50%] px-3">
                 <ReviewCard review={review} />
              </div>)}
          </div>
        </div>
        ...
        <div className="flex justify-center mt-6">
          <div className="flex items-center gap-2">
            {scrollSnaps.map((_, index: number) => <div key={index} className={`w-6 h-1 rounded-full transition-all duration-300 ${index === selectedIndex ? 'bg-gray-700 w-12' : 'bg-gray-400 opacity-50'}`} />)}
          </div>
        </div>
      </div>
    </div>
...
const GoogleReviews: React.FC = () => {
  ...
  const reviews = [{
    ...
    name: "Helena Costa",
    rating: 5,
    comment: "O Mário foi um consultor muito prestável na compra do meu apartamento e do meu namorado. Além de saber exatamente o que procurávamos mostrou-se sempre disponível para toda e qualquer dúvida. Recomendo plenamente o seu serviço.",
    avatar: null
  }, {
    id: 2,
    name: "Fike",
    rating: 5,
    comment: "Gostei da disponibilidade 😃",
    avatar: fikeAvatar
  }, {
    id: 3,
    name: "Tiago Brava",
    rating: 5,
    comment: "Confiança e dedicação!",
    avatar: tiagoAvatar
  }, {
    id: 4,
    name: "Fábia Silva",
    rating: 5,
    comment: "Serviço 5 estrelas.",
    avatar: fabiaAvatar
  }, {
    id: 5,
    name: "Luís Areal",
  ...
  return isDesktop ? <DesktopLayout {...componentProps} /> : <MobileLayout {...componentProps} />;
};
